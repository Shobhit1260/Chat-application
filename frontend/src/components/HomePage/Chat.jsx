...
function Chat() {
  ...
  }, [me?._id, userSelected?._id,sendmsg]);
        ...
        <div className="z-0 flex flex-col justify-between w-[600px] h-full backdrop-blur bg-white/10 rounded-r-lg">
          ...
          <div className="flex flex-col gap-2 overflow-y-scroll h-[500px] px-2 py-4">
            {chatHistory.map((msg, index) => (
              isGroup ? (console.log("msg", msg),
                <div key={msg._id || index} className={`flex gap-4 ${msg.sender._id === me?._id ? "justify-end" : "justify-start"}`}>
                  <img src={msg.sender.picture} alt="profile" className="aspect-[1/1] rounded-full w-8 h-8"/>
                  <div
              ...
              ) : (console.log("msg", msg),console.log("me",me),
                <div key={msg._id || index} className={`flex gap-4 ${msg.sender._id === (me?._id) ? "justify-end" : "justify-start"}`}>
                  <img src={msg.sender?.picture} alt="profile" className="aspect-[1/1] rounded-full w-8 h-8"/>
                  <div
                    className={`max-w-[70%] px-4 py-2 rounded-xl ${
